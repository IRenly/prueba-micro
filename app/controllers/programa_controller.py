...
from app.models.programa_model import ProgramaDB, Programa
from fastapi import HTTPException, status

def obtener_programas(db: Session):
    return db.query(ProgramaDB).all()

def obtener_programa_por_id(id: int, db: Session):
    programa = db.query(ProgramaDB).filter(ProgramaDB.id == id).first()
    if not programa:
